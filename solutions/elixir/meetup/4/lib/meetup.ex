defmodule Meetup do
  ...
  defp first_day(year, month, _schedule), do: Date.new!(year, month, 1)

  @spec find_first_weekday(Date.t(), weekday) :: Date.t()
  defp find_first_weekday(date, weekday) do
  ...
  defp find_target_date(date, :first), do: date
  defp find_target_date(date, :second), do: Date.shift(date, day: 7)
  defp find_target_date(date, :third), do: Date.shift(date, day: 14)
  defp find_target_date(date, :fourth), do: Date.shift(date, day: 21)
  defp find_target_date(date, :last), do: date
  defp find_target_date(date, :teenth), do: date
end
