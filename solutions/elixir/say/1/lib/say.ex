defmodule Say do
  ...
  def in_english(number) when number < 1_000_000 do
    ...
    {:ok, thousands} = in_english(t)

    if rest == 0 do
      {:ok, "#{thousands} thousand"}
    else
      {:ok, rest_val} = in_english(rest)
      {:ok, "#{thousands} thousand #{rest_val}"}
    end
  end
  ...
  def in_english(number) when number < 1_000_000_000 do
    ...
    rest = rem(number, 1_000_000)

    {:ok, millions} = in_english(m)

    if rest == 0 do
      {:ok, "#{millions} million"}
    else
      {:ok, rest_val} = in_english(rest)
      {:ok, "#{millions} million #{rest_val}"}
    end
  end
  ...
  def in_english(number) when number < 1_000_000_000_000 do
    ...
    rest = rem(number, 1_000_000_000)

    {:ok, billions} = in_english(b)

    if rest == 0 do
      {:ok, "#{billions} billion"}
    else
      {:ok, rest_val} = in_english(rest)
      {:ok, "#{billions} billion #{rest_val}"}
    end
  end
end
